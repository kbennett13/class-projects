...
class Slider {
  ...
  Slider(int start, int end, int defaultLoc, int x, int y, int len) {
    this.name = name;
  ...
  void drawSlider() {
    ...
    fill(200);
    rectMode(CENTER);
    int sliderX = (int)(len * (loc-start) / (float)(end-start));
    ...
    textAlign(LEFT);
    textSize(12);
    text(start, x-textWidth(Integer.toString(start))-5, y+textAscent()/3);
    text(end, x+len+5, y+textAscent()/3);
    text(loc, x+sliderX-textWidth(Integer.toString(loc))/2, y+20);
  }
  
  /**
   * Return the value that the slider is currently pointing to.
   */
  int getSize() { return loc; }
    ...
    if (!dragging) {
      if (my > y-8 && my < y+8) {
        ...
        if (mx > currX-4 && mx < currX+4) {
          ...
          loc = start + (int)((end - start) * diff / len + 0.5);
          return loc;
        }
      }
    }
   ...
   */
  int mouseDragged(int mx, int my) {
    if (dragging) {
      float diff = (float)(mx - x);
      loc = start + (int)((end - start) * diff / len + 0.5);
      if (loc < start) loc = start;
